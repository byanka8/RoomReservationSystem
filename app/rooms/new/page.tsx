...
export default function NewRoomPage() {
  ...
  if (!user) {
    return (
      <div className="min-h-screen bg-slate-50 flex items-center justify-center px-4 py-12">
        <div className="rounded-3xl border border-slate-200 bg-white/95 p-8 shadow-2xl shadow-slate-200/40">
          <p className="text-sm text-slate-500">Please login to create a room.</p>
        </div>
      </div>
    );
  }
      ...
      <main className="mx-auto max-w-4xl px-4 py-10 sm:px-6 lg:px-8">
        <div className="mb-6 flex flex-col gap-4 rounded-[1.75rem] border border-slate-200 bg-white/95 p-8 shadow-2xl shadow-slate-200/40 sm:flex-row sm:items-center sm:justify-between">
          <div>
            ...
            <h1 className="mt-3 text-3xl font-semibold tracking-tight text-slate-900">Create room</h1>
            <p className="mt-2 text-sm leading-6 text-slate-500">Add a new room to the reservation system.</p>
          </div>
          <Link
            href="/rooms"
